...
function taskCreater(){
  for(let index in textArray){
    ...
    task[`${index}`].div.circle.checkIcon.setAttribute("src", "./Images/icon-check.svg");
    ...
    task[`${index}`].div.cross.image.setAttribute("src", "./Images/icon-cross.svg");
    task[`${index}`].div.cross.appendChild(task[`${index}`].div.cross.image);
    // hover cross image
    task[`${index}`].div.cross.hoverImage = document.createElement("img");
    task[`${index}`].div.cross.hoverImage.setAttribute("src", "./Images/cross-hover.svg");
